...
func TestParseTarget(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "custom port flag overrides",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			config, err := parseTarget(tt.target, tt.port)
...
func TestGenerateKeyPair(t *testing.T) {
	...
	// Check it starts with ssh-ed25519
	pubKeyStr := string(pubKeyData)
	if len(pubKeyStr) < 11 || pubKeyStr[:11] != "ssh-ed25519" {
		t.Error("generateKeyPair() public key doesn't start with ssh-ed25519")
	}
}